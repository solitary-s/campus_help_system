                    ...
                    <div class="box box-info box-info-search" style="display: none;">
                        ...
                        <div class="box-body">
                            <div class="row form-horizontal">
                                ...
                                <div class="col-xs-12 col-sm-3">
                                    <div class="form-group">
                                        <label for="used_detail_id" class="col-sm-4 control-label">订单编号</label>

                                        <div class="col-sm-8">
                                            <input id="used_detail_id" class="form-control" placeholder="订单编号" />
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    $(function () {
        var _columns = [
            ...
            {"data": "usedDetail.used_detail_id"},
            {"data": "publish_id"},
            {"data": "accept_id"},
            {"data": "order_status"},
            {"data": "usedDetail.used_title"},
            {"data": "usedDetail.used_publishtime",
            ...
            {
                "data": function (row, type, val, meta) {
                    var detailUrl = "/used/detail?used_detail_id=" + row.usedDetail.used_detail_id;
                    var deleteUrl = "/used/delete";
                    return '<button type="button" class="btn btn-sm btn-default" onclick="App.showDetail(\'' + detailUrl + '\');"><i class="fa fa-search"></i> 查看</button>&nbsp;&nbsp;&nbsp;' +
                        '<a href="/used/formUpdate?used_detail_id=' + row.usedDetail.used_detail_id + '" type="button" class="btn btn-sm btn-primary">' +
                        '<i class="fa fa-edit"></i> 编辑</a>&nbsp;&nbsp;&nbsp;' +
                        '<button type="button" class="btn btn-sm btn-danger" onclick="App.deleteSingle(\'' + deleteUrl + '\', \'' + row.usedDetail.used_detail_id+ '\')">' +
                        '<i class="fa fa-trash-o"></i> 删除</button>';
                }
            }
        ];
        _dataTable = App.initDataTables("/used/page", _columns);
    });

    function search() {
        var publish_id = $("#publish_id").val();
        var accept_id = $("#accept_id").val();
        var used_detail_id = $("#used_detail_id").val();
